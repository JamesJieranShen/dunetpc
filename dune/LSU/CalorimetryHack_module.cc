//////////////////////////////////////////////////
//
// CalorimetryHack class
//
//  Justin Hugon hacked this from LArReco version to support tpIndices
//  on 2018-12-20
//
...
namespace calo {
  ...
  class CalorimetryHack : public art::EDProducer {
    
  public:
    
    explicit CalorimetryHack(fhicl::ParameterSet const& pset); 
    virtual ~CalorimetryHack();
...
//-------------------------------------------------
calo::CalorimetryHack::CalorimetryHack(fhicl::ParameterSet const& pset)
...
//-------------------------------------------------
calo::CalorimetryHack::~CalorimetryHack()
{
  
}

//-------------------------------------------------
void calo::CalorimetryHack::beginJob()
{
  return;
}

//------------------------------------------------------------------------------------//
void calo::CalorimetryHack::produce(art::Event& evt)
{ 
  ...
  for(size_t trkIter = 0; trkIter < tracklist.size(); ++trkIter){   
      ...
      for (size_t itp = 0; itp < tracklist[trkIter]->NumberTrajectoryPoints(); ++itp){
        ...
        if (tpcid.isValid) {
          try{
            ...
            mf::LogWarning("CalorimetryHack") << "caught exception " 
                                          << e << "\n setting pitch (C) to "
                                          << util::kBogusD;
            fTrkPitch = 0;
          }
          ...
          for (size_t ii = 0; ii<vhit.size(); ++ii){
            if (vhit[ii].key() == allHits[hits[ipl][ihit]].key()){
              ...
              if (vmeta[ii]->Index()>=tracklist[trkIter]->NumberTrajectoryPoints()){
                throw cet::exception("CalorimetryHack_module.cc") << "Requested track trajectory index "<<vmeta[ii]->Index()<<" exceeds the total number of trajectory points "<<tracklist[trkIter]->NumberTrajectoryPoints()<<" for track index "<<trkIter<<". Something is wrong with the track reconstruction. Please contact tjyang@fnal.gov";
              }
              ...
              auto loc = tracklist[trkIter]->LocationAtPoint(tpIndex);
              xyz3d[0] = loc.X();
              xyz3d[1] = loc.Y();
              xyz3d[2] = loc.Z();
              break;
	...
	if (channelStatus.IsBad(channel)){
	  MF_LOG_DEBUG("CalorimetryHack") << "Found dead wire at Plane = " << plane 
					 << " Wire =" << iw;
...
void calo::CalorimetryHack::GetPitch(art::Ptr<recob::Hit> hit, std::vector<double> trkx, std::vector<double> trky, std::vector<double> trkz, std::vector<double> trkw, std::vector<double> trkx0, double *xyz3d, double &pitch, double TickT0){
...
//    xyz = isp->second->XYZ();
  ...
  if (np>=2){//at least two points
	...
	xs->Fit("pol1","Q");
      ...
      mf::LogWarning("CalorimetryHack::GetPitch") <<"Fitter failed";
      xyz3d[0] = vx[0];
    }
    delete xs;
...
namespace calo{
  ...
  DEFINE_ART_MODULE(CalorimetryHack)
