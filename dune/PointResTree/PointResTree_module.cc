 ...
 * @brief Retrieve and calculate relevant infromation from the art reco file. Save all information in a tree.
 * @version 2.1
 * @date 2021-11-29
...
{
    /**
     * @brief Faster version of ParticleInventoryService, used to the generator associated with a reco track.
     * 
     */
    struct genFinder
    {
    ...
    private:
        //NOT IN TREE:
        genFinder *gf;
        track_loc primary_trk_loc; ///< Keeping track of primary track's id in its producer.
        ...
        // histograms
        TH1F *vertex_reco_distance; ///< Distance between the SP vertex and truth position
        ...
        Double_t truth_en_deposition, truth_charge_deposition; ///< G4 deposition information
        ...
        ///< Vector of collection plane charge over different distance cut from veretx.
        ...
        std::vector<Double_t> trk_start_x, trk_start_y, trk_start_z; ///< Starting Position of all vallid tracks.
        std::vector<Double_t> trk_end_x, trk_end_y, trk_end_z;       ///< Ending Position of all vallid tracks.
        ...
        /**
             * @brief Set Neutrino and Electron truths to tree variables, assuming marley generator.
             * 
        ...
        /**
             * @brief Set Neutrino and Electron truths to tree variables, assuming largeant generator.
             * 
             * @param event art event.
             */
        ...
        /**
             * @brief Given accumulated charges on the U, V, and Z planes, does drift correction and reco energy of the electron.
             * @param event art event.
             */
        void calculate_electron_energy(art::Event const &event);

        /**
             * @brief Reads the track direction information, calculate the direction of the primary track.
             * Most importantly, this function carries out daughter flipping.
             */
        void calculate_electron_direction();

        /**
             * @brief Determine if a hit should be included in the energy sum.
             * If there are 3D reco information (SpacePoints) associated with the hit, use that as the track location.
             * If there are no spacepoints, carry out a 2D reconstruction using drift time and the center of the wire segment.
             * @param event art event.
             ...
             */
        Bool_t distance_cut(art::Event const &event, recob::Hit const &hit, std::vector<art::Ptr<recob::SpacePoint>> spacepoints);
...
void dune::PointResTree::analyze(art::Event const &event)
{
    ...
    // use spacepoint info to reconstruct an approximate vertex. This vertex is then used to find correct tracks by
    // scanning tracks close to it.
    auto sp_vertex = find_vertex_sp(event);
    auto sp_distance_th = 120; // if track is further away from sp_vertex than this, it gets thrown out
    ...
    if (fCollectTracks)
    {
        ...
        for (std::string track_label : fTrackModuleLabel)
        {
            ...
            { // using index loop to get track idx
                ...
                Double_t trk_weight = charge * (track_label == "pandoraShower" ? 10.0 : 1.0); // prioritize showers
...
{
    Bool_t found_nu, found_e = kFALSE;
    auto particleHandle = event.getValidHandle<std::vector<simb::MCParticle>>(fSimulationLabel);
    NParticles = 0;
    ...
    for (auto const &particle : (*particleHandle))
    {
        ...
        if (particle.Process() == "primary" &&
            (particle.PdgCode() == 12 || particle.PdgCode() == 14 || particle.PdgCode() == 16 || particle.PdgCode() == -12 || particle.PdgCode() == -14 || particle.PdgCode() == -16))
        {
            found_nu = kTRUE;
