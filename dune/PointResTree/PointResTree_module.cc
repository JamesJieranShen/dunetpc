...
public:
  ...
  {
    std::sort (
        this->track_id_map.begin (), this->track_id_map.end (),
...
private:
  ...
  Double_t charge_U, charge_V, charge_Z; ///< Total charge on each plane (ADC).
  ...
  /**
   * @brief Set Neutrino and Electron truths to tree variables, assuming marley
   * generator.
   *
   * @param event art event.
   */
  void writeMCTruths_marley (art::Event const &event);

  /**
   * @brief Set Neutrino and Electron truths to tree variables, assuming
   * largeant generator.
   *
   * @param event art event.
   */
  void writeMCTruths_largeant (art::Event const &event);
...
void
dune::PointResTree::reconfigure (fhicl::ParameterSet const &parameterSet)
{
  ...
  fHitToSpacePointLabel
      = parameterSet.get<std::string> ("HitToSpacePointLabel");
...
void
dune::PointResTree::beginJob ()
{
  art::ServiceHandle<art::TFileService> tfs;

  vertex_reco_distance = tfs->make<TH1F> (
      "vertex_reco_distance",
      "Distance between reconstructed vertex (vis SP) and true vertex", 1000,
      0, 1000);
...
void
dune::PointResTree::endJob ()
{
  std::cout << "Ending Job" << std::endl;
} // endJob()

void
dune::PointResTree::analyze (art::Event const &event)
{
  // ana begin
  reset_variables ();
  gf = new genFinder ();
  ...
  if (fCollectTracks)
    {
      ...
      for (std::string track_label : fTrackModuleLabel)
        {
          ...
          art::FindManyP<recob::Hit> hitsFromTracks (trk_list, event,
                                                     track_label);
          trk_count += trk_list->size ();
          for (int i = 0; i < (int)trk_list->size (); i++)
            { // using index loop to get track idx
              recob::Track const &trk = trk_list->at (i);
              XYZVector trk_start (trk.Start ().X (), trk.Start ().Y (),
                                   trk.Start ().Z ());
              XYZVector trk_end (trk.End ().X (), trk.End ().Y (),
                                 trk.End ().Z ());
              ...
              Double_t trk_weight = charge
                                    * (track_label == "pandoraShower"
                                           ? 10.0
                                           : 1.0); // prioritize showers
              ...
              if (trk.Length () > 100)
                {
                  ...
                  while (idx != iLast)
                    {
                      auto current_loc
                          = trk.Trajectory ().LocationAtPoint (idx);
                      auto next_idx = trk.Trajectory ().NextValidPoint (idx);
                      auto next_loc
                          = trk.Trajectory ().LocationAtPoint (next_idx);
      ...
      if (DEBUG_MSG)
        {
          std::cout << "Number of Tracks cut: " << trk_count - NTrks
                    << std::endl;
          std::cout << "NTrks: " << NTrks << std::endl;
          std::cout << "SP Vertex Loc: " << sp_vertex.X () << " "
                    << sp_vertex.Y () << " " << sp_vertex.Z () << std::endl;
          if (NTrks == 0)
            std::cout << "No Tracks FOUND!!" << std::endl;
          else
            {
              std::cout << "Primary Track Start: "
                        << trk_start_x.at (primary_trk_id) << " "
...
void
dune::PointResTree::reset_variables ()
{
...
void
dune::PointResTree::writeMCTruths_marley (art::Event const &event)
{
  ...
  for (auto const &particle : (*particleHandle_nu))
    {
      if (particle.NeutrinoSet ())
        { // entry is a neutrino
          ...
          truth_nu_dir.SetXYZ (neutrino.Px (), neutrino.Py (), neutrino.Pz ());
      ...
      if (particle.Process () == "primary" && particle.PdgCode () == 11)
        {
          ...
          truth_e_dir.SetXYZ (particle.Px (), particle.Py (), particle.Pz ());
...
void
dune::PointResTree::writeMCTruths_largeant (art::Event const &event)
{
  Bool_t found_nu, found_e = kFALSE;
  auto particleHandle = event.getValidHandle<std::vector<simb::MCParticle> > (
      fSimulationLabel);
  NParticles = 0;
  ...
  for (auto const &particle : (*particleHandle))
    {
      ...
      if (gf->get_gen (particle.TrackId ()) != "generator")
        continue; // do not track radiologicals
      // neutrino
      if (particle.Process () == "primary"
          && (particle.PdgCode () == 12 || particle.PdgCode () == 14
              || particle.PdgCode () == 16 || particle.PdgCode () == -12
              || particle.PdgCode () == -14 || particle.PdgCode () == -16))
        {
          found_nu = kTRUE;
          ...
          truth_nu_dir.SetXYZ (particle.Px (), particle.Py (), particle.Pz ());
          truth_nu_dir.SetMag (1.0);
        }
      // electron
      if (particle.Process () == "primary" && particle.PdgCode () == 11)
  ...
  auto channel_list = event.getValidHandle<std::vector<sim::SimChannel> > (
      fSimChannelModuleLabel);
  ...
  if (NParticles != 1)
    std::cerr << "WARNING: More than one primary electron is found"
              << std::endl;
  if (!found_nu)
    std::cerr << "WARNING: Neutrino MC info not found" << std::endl;
  if (!found_e)
...
void
dune::PointResTree::calculate_electron_energy (art::Event const &event)
{
  // use collection plane charge for reconstruction
  Double_t charge_raw = charge_Z;
  // drift correction
  if (NTrks == 0 || primary_trk_id == -1)
    { // no tracks found, do not do drift correction
  ...
  if (DEBUG_MSG)
    std::cout << "primary_trk_label is " << primary_trk_label
              << " hitsFromTracks size: " << hitsFromTracks.size ()
              << std::endl;
  ...
  while (totalPEs.size () != 0)
    {
      ...
      if (current_drift_time > 0 && current_drift_time < 2400.0)
        { // drift time found
...
void
dune::PointResTree::calculate_electron_direction ()
{
  ...
  for (int i = 0; i < NTrks; i++)
    {
      ...
      current_trk.SetXYZ (trk_start_x.at (i), trk_start_y.at (i),
                          trk_start_z.at (i));
      difference_start = current_trk - primary_start;
      difference_end = current_trk - primary_end;
      sum_cos_forward += ROOT::Math::VectorUtil::CosTheta (difference_start,
                                                           primary_forward);
      sum_cos_backward += ROOT::Math::VectorUtil::CosTheta (difference_end,
                                                            primary_backward);
    }
...
Bool_t
dune::PointResTree::distance_cut (
    ...
    std::vector<art::Ptr<recob::SpacePoint> > spacepoints)
{
  ...
  if (spacepoints.size ())
    ...
    { // no spacepoint found, do 2D reconstruction (X, Z)
      ...
      plane.DriftPoint (
          pos, -abs (detProperties.ConvertTicksToX (
...
void
dune::PointResTree::write_multi_distance (
    art::Event const &event, recob::Hit const &hit,
    std::vector<art::Ptr<recob::SpacePoint> > spacepoints)
{
  if (primary_trk_id < 0)
  ...
  for (int i = 0; i < 20; i++)
    {
      double cut
          = 14.0 * (i + 1); // each bin signifies a cut applied at 14*bin
      if (distance < cut)
...
XYZVector
dune::PointResTree::find_vertex_sp (art::Event const &event)
{
  ...
  for (int i = 0; i < N_spacepoint; i++)
    {
      ...
      if (sp_brightness_queue.size () < queue_size)
        {
          sp_brightness_queue.push (
              AVGAMP_SP (avg_amplitude, sp_handle->at (i)));
        }
